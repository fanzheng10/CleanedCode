...
def numJobs(u='fzheng'):
    while True:
        out = sub.check_output('qstat -u ' + u + ' | grep "" | wc -l', stderr=sub.STDOUT, shell=True)
        try:
            njobs = int(out) - 2
            break
        except:
...
def createLocalSpace(username = 'fzheng'):
    while True:
        rint = str(random.randint(0, 1000000)).rjust(6, '0')
        ldir = '/data/scratch/'+ username + '/' + rint
